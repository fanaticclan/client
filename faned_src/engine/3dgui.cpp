...
VARP(guialpha, 0, 255, 255); // Fanatic Edition
...
struct gui : g3d_gui
{
        ...
        if(visible())
        {
            ...
            if(overlaid)
            {
                if(hit)
                {
                    ...
                    glColor3f(0.5f, 0.5f, 0.5f); // Fanatic Edition
                    rect_(xi, yi, xs, ys);
                    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                    glEnable(GL_TEXTURE_2D);
                    defaultshader->set();
                }
                if(!overlaytex) overlaytex = textureload("faned/data/guioverlay.png", 3); // Fanatic Edition
                glColor3fv(light.v);
    ...
    void slider(int &val, int vmin, int vmax, int color, const char *label)
    {
        ...
        if(visible())
        {
            ...
            if(hit) color = 0x50CFE5; // Fanatic Edition
    ...
    {	
        ...
        if(visible() && !layoutpass)
        {
            ...
            glDisable(GL_TEXTURE_2D);
            glDisable(GL_BLEND);
            if(editing) glColor3f(0.5, 0.5, 0.5); // Fanatic Edition
    ...
    void icon_(Texture *t, bool overlaid, int x, int y, int size, bool hit)
    {
        ...
        x += int((size-xs)/2);
        y += int((size-ys)/2);
        const vec color = vec(1, 1, 1); // Fanatic Edition
    ...
    {		
        if(visible())
        {
            if(!slidertex) slidertex = textureload("faned/data/guislider.png", 3); // Fanatic Edition
    ...
    int button_(const char *text, int color, const char *icon, bool clickable, bool center)
    {
        ...
        if(visible())
        {
            ...
            if(hit && clickable) color = 0x555555; // Fanatic Edition
            if(hit && clickable && mousebuttons&G3D_UP) playsound(S_GUIBUTTON); // Fanatic Edition
            int x = curx;
    ...
    {
        if(!skintex) skintex = textureload("faned/data/guiskin.png", 3); // Fanatic Edition
        ...
        {	
            ...
            glColor4f(j ? light.x : 1.0f, j ? light.y : 1.0f, j ? light.z : 1.0f, passes<=1 || j ? ((float)guialpha/255) : (((float)guialpha/255))/2); // Fanatic Edition
            loopi(n)
            {
