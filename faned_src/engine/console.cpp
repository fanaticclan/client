...
VARP(consmoothfade, 0, 1, 1); // Fanatic Edition
VARP(contimestamp, 0, 1, 2); // Fanatic Edition
...
VARP(fullconsize, 0, 70, 100); // Fanatic Edition
...
int drawconlines(int conskip, int confade, int conwidth, int conheight, int conoff, int filter, int y = 0, int dir = 1)
{
    ...
    loopi(numl)
    {
        ...
        // Start: Fanatic Edition
        string line;
        if(contimestamp >= 1 && (fullconsole || contimestamp == 2) && conlines[idx].time && conlines[idx].time[0]) formatstring(line)("\fs\f4[%s]\fr %s", conlines[idx].time, conlines[idx].line);
...
VARP(confix, 0, 600, 10000); // Fanatic Edition
...
int renderconsole(int w, int h, int abovehud)
{
    int conpad = fullconsole ? 0 : FONTH/4,
        ...
        conwidth = w - 2*(conpad + conoff) - (fullconsole ? 0 : game::clipconsole(w, h)) - (fullconsole ? 0 : confix); // Fanatic Edition
...
void inputcommand(char *init, char *action = NULL, char *prompt = NULL, char *flags = NULL)
{
...
// Start: Fanatic Edition
const char *historyfile() { return "faned_history.log"; }
