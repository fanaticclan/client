...
#endif
{
    ...
    void drawhud(fpsent *d, int w, int h)
    {
        ...
        if(strstr(radar, "sauerenhanced")) settexture("faned/hud/se_hud_radar.png", 3);
        else if(strstr(radar, "sauerbraten")) settexture("faned/hud/radar_sauerbraten.png", 3);
        else if(strstr(radar, "tesseract")) settexture("faned/hud/radar_tesseract.png", 3);
        else if(strstr(radar, "reveladerevolution")) settexture("faned/hud/radar_reveladerevolution.png", 3);
        else if(strstr(radar, "k1p57a")) settexture("faned/hud/k1p57a_radar.png", 3);
        drawradar(x - roffset, y - roffset, rsize);
        if(compass)
        {
            settexture("faned/hud/compass_tesseract.png", 3);
            glPushMatrix();
        ...
        // Start: Fanatic Edition
        if(!joinred) settexture(team == collectteambase(player1->team) ? "packages/hud/blip_red_skull.png" : "packages/hud/blip_blue_skull.png", 3);
        else settexture(team == collectteambase(player1->team) ? "packages/hud/blip_blue_skull.png" : "packages/hud/blip_red_skull.png", 3);
        // End: Fanatic Edition
        loopv(players)
        {
            fpsent *o = players[i];
        ...
        if(d->state == CS_DEAD)
        {
            ...
            if(wait>=0)
            {
                ...
                // Start: Fanatic Edition
                if(!joinred) draw_textf("%s%d", (x+s/2)/2-(wait>=10 ? 28 : 16), (y+s/2)/2-32, flash ? "\f3" : "", wait);
                else draw_textf("%s%d", (x+s/2)/2-(wait>=10 ? 28 : 16), (y+s/2)/2-32, flash ? "" : "\f3", wait);
                // End: Fanatic Edition
                glPopMatrix();
            }
        }
    ...
    void rendergame()
    {
        ...
        loopv(bases)
        {
            base &b = bases[i];
            const char *basename;
            if(!joinred) basename = b.team==team ? "base/blue" : "base/red";
            else basename = b.team==team ? "base/red" : "base/blue";
            ...
            if(!joinred) regular_particle_flame(PART_FLAME, vec(b.tokenpos.x, b.tokenpos.y, b.tokenpos.z - 4.5f), fradius, fheight, b.team==team ? 0x2020FF : 0x802020, 3, 2.0f);
            else regular_particle_flame(PART_FLAME, vec(b.tokenpos.x, b.tokenpos.y, b.tokenpos.z - 4.5f), fradius, fheight, b.team==team ? 0x802020 : 0x2020FF, 3, 2.0f);
            ...
            if(!joinred) rendermodel(&b.light, b.team==team ? "skull/blue" : "skull/red", ANIM_MAPMODEL|ANIM_LOOP, tokenpos, lastmillis/10.0f, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED, NULL, NULL, 0, 0, alpha);
            else rendermodel(&b.light, b.team==team ? "skull/red" : "skull/blue", ANIM_MAPMODEL|ANIM_LOOP, tokenpos, lastmillis/10.0f, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED, NULL, NULL, 0, 0, alpha);
            formatstring(b.info)("%d", totalscore(b.team));
            vec above(b.tokenpos);
            above.z += TOKENHEIGHT;
            if(!joinred) { if(b.info[0]) particle_text(above, b.info, PART_TEXT, 1, b.team==team ? 0x6496FF : 0xFF4B19, 2.0f); }
            else { if(b.info[0]) particle_text(above, b.info, PART_TEXT, 1, b.team==team ? 0xFF4B19 : 0x6496FF, 2.0f); }
        }
        loopv(tokens)
        {
            token &t = tokens[i];
            ...
            if(!joinred) rendermodel(&t.light, t.team == team || (t.team < 0 && -t.team != team) ? "skull/blue" : "skull/red", ANIM_MAPMODEL|ANIM_LOOP, p, lastmillis/10.0f, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED);
            else rendermodel(&t.light, t.team == team || (t.team < 0 && -t.team != team) ? "skull/red" : "skull/blue", ANIM_MAPMODEL|ANIM_LOOP, p, lastmillis/10.0f, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED);
        }
        ...
        loopv(players)
        {
            ...
            loopj(d->tokens)
            {
                if(!joinred) rendermodel(&light, dteam != team ? "skull/blue" : "skull/red", ANIM_MAPMODEL|ANIM_LOOP, pos, d->yaw+90, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED);
                else rendermodel(&light, dteam != team ? "skull/red" : "skull/blue", ANIM_MAPMODEL|ANIM_LOOP, pos, d->yaw+90, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED);
                pos.z += TOKENHEIGHT + 1;
            }
        }        
    }
    ...
    void baseexplosion(int i, int team, const vec &loc)
    {
        ...
        {
            if(team==collectteambase(player1->team)) { fcolor = 0x802020; color = vec(0.25f, 0.25f, 1); }
            else { fcolor = 0x2020FF; color = vec(1, 0.25f, 0.25f); }
            particle_fireball(loc, 30, PART_EXPLOSION, -1, fcolor, 4.8f);
            adddynlight(loc, 35, color, 900, 100);
            particle_splash(PART_SPARK, 150, 300, loc, fcolor, 0.24f);
        }
    ...
    void baseeffect(int i, int team, const vec &from, const vec &to, bool showfrom = true, bool showto = true)
    {
        ...
        // Start: Fanatic Edition
        if(!joinred) particle_flare(from, to, 600, PART_LIGHTNING, team==collectteambase(player1->team) ? 0x2222FF : 0xFF2222, 1.0f);
        else particle_flare(from, to, 600, PART_LIGHTNING, team==collectteambase(player1->team) ? 0xFF2222 : 0x2222FF, 1.0f);
        // End: Fanatic Edition
    }
    ...
    token *droptoken(fpsent *d, int id, const vec &o, int team, int yaw, int n)
    {
        ...
        if(d) 
        {
            ...
            else
            {
                if(!n)
                {
                    particle_fireball(d->o, 4.8f, PART_EXPLOSION, 500, team==collectteambase(player1->team) ? 0x802020 : 0x2020FF, 4.8f);
                    particle_splash(PART_SPARK, 50, 250, d->o, team==collectteambase(player1->team) ? 0x802020 : 0x2020FF, 0.24f);
                }
                particle_flare(d->o, vec(t.o.x, t.o.y, t.o.z + 0.5f*(TOKENHEIGHT + 1)), 500, PART_LIGHTNING, team==collectteambase(player1->team) ? 0xFF2222 : 0x2222FF, 1.0f); 
            }
            // End: Fanatic Edition
        }
        return &t;
    ...
    void stealtoken(fpsent *d, int id, const vec &o, int team, int yaw, int n, int basenum, int enemyteam, int score)
    {
        ...
        if(bases.inrange(basenum))
        {
            ...
            // Start: Fanatic Edition
            if(!joinred) { if(t) particle_flare(b.tokenpos, vec(t->o.x, t->o.y, t->o.z + 0.5f*(TOKENHEIGHT + 1)), 500, PART_LIGHTNING, team==collectteambase(player1->team) ? 0x2222FF : 0xFF2222, 1.0f); }
            else { if(t) particle_flare(b.tokenpos, vec(t->o.x, t->o.y, t->o.z + 0.5f*(TOKENHEIGHT + 1)), 500, PART_LIGHTNING, team==collectteambase(player1->team) ? 0xFF2222 : 0x2222FF, 1.0f); }
            // End: Fanatic Edition
        }
    }
