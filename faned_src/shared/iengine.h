...
extern void crosshairbump();    // Fanatc Edition
...
extern void draw_textfa(const char *fstr, int a, int left, int top, ...);    // Fanatc Edition;
...
extern void particle_trail_spin(int type, int fade, const vec &from, const vec &to, int color = 0xFFFFFF, float size = 1.0f, int gravity = 20, float radius = 3.50f, float period = 20.0f);
...
extern void renderclient(dynent *d, const char *mdlname, modelattach *attachments, int hold, int attack, int attackdelay, int lastaction, int lastpain, float fade = 1, bool ragdoll = false, bool thirdperson = false);
