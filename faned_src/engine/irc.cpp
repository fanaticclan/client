...
void ircestablish(ircnet *n)
{
    ...
    {
        conoutf(CON_IRC, "\f9FanEd\f7::irc: looking up %s:[%d]...", n->serv, n->port);
        if(!resolverwait(n->serv, &n->address))
        {
            conoutf(CON_IRC, "\f9FanEd\f7::irc: unable to resolve %s:[%d]...", n->serv, n->port);
            n->state = IRC_DISC;
            return;
        }
    }
    ...
    if(*n->ip && enet_address_set_host(&address, n->ip) < 0) conoutf(CON_IRC, "\f9FanEd\f7::irc: failed to bind address: %s", n->ip);
    n->sock = enet_socket_create(ENET_SOCKET_TYPE_STREAM);
    if(n->sock != ENET_SOCKET_NULL && *n->ip && enet_socket_bind(n->sock, &address) < 0)
    {
        conoutf(CON_IRC, "\f9FanEd\f7::irc: failed to bind connection socket: %s", n->ip);
    ...
    n->state = IRC_ATTEMPT;
    conoutf(CON_IRC, "\f9FanEd\f7::irc: connecting to %s:[%d]...", n->serv, n->port);
}
...
void ircnewnet(int type, const char *name, const char *serv, int port, const char *nick, const char *ip, const char *passkey)
{
    ...
    if(m)
    {
        if(m->state != IRC_DISC) conoutf(CON_IRC, "\f9FanEd\f7::irc: ircnet %s already exists", m->name);
        else ircestablish(m);
        return;
    }
    ...
    conoutf(CON_IRC, "\f9FanEd\f7::irc: added irc %s %s (%s:%d) [%s]", type == IRCT_RELAY ? "relay" : "client", name, serv, port, nick);
}
...
ICOMMAND(ircserv, "ss", (const char *name, const char *s), {
    ...
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s current server is: %s", n->name, n->serv); return; }
    copystring(n->serv, s);
});
ICOMMAND(ircport, "ss", (const char *name, const char *s), {
    ircnet *n = ircfind(name);
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(!s || !*s || !parseint(s)) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s current port is: %d", n->name, n->port); return; }
    n->port = parseint(s);
});
ICOMMAND(ircnick, "ss", (const char *name, const char *s), {
    ircnet *n = ircfind(name);
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s current nickname is: %s", n->name, n->nick); return; }
    copystring(n->nick, s);
});
ICOMMAND(ircbind, "ss", (const char *name, const char *s), {
    ircnet *n = ircfind(name);
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s currently bound to: %s", n->name, n->ip); return; }
    copystring(n->ip, s);
});
ICOMMAND(ircpass, "ss", (const char *name, const char *s), {
    ircnet *n = ircfind(name);
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s current password is: %s", n->name, n->passkey && *n->passkey ? "<set>" : "<not set>"); return; }
    copystring(n->passkey, s);
});
ICOMMAND(ircauth, "sss", (const char *name, const char *s, const char *t), {
    ircnet *n = ircfind(name);
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(!s || !*s || !t || !*t) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s current authority is: %s (%s)", n->name, n->authname, n->authpass && *n->authpass ? "<set>" : "<not set>"); return; }
...
ICOMMAND(ircconnect, "s", (const char *name), {
    ircnet *n = ircfind(name);
    if(!n) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name); return; }
    if(n->state != IRC_DISC) { conoutf(CON_IRC, "\f9FanEd\f7::irc: ircnet %s is already connected", n->name); return; }
    ircestablish(n);
});
...
bool ircjoin(ircnet *n, ircchan *c)
{
    ...
    if(n->state == IRC_DISC)
    {
        conoutf(CON_IRC, "\f9FanEd\f7::irc: ircnet %s is not connected", n->name);
        return false;
    }
...
bool ircenterchan(ircnet *n, const char *name)
{
    ...
    if(!c)
    {
        conoutf(CON_IRC, "\f9FanEd\f7::irc: ircnet %s has no channel called %s ready", n->name, name);
        return false;
    }
    return ircjoin(n, c);
}
...
bool ircnewchan(int type, const char *name, const char *channel, const char *friendly, const char *passkey, int relay)
{
    ...
    if(!n)
    {
        conoutf(CON_IRC, "\f9FanEd\f7::irc: no such ircnet: %s", name);
        return false;
    }
    ircchan *c = ircfindchan(n, channel);
    if(c)
    {
        conoutf(CON_IRC, "\f9FanEd\f7::irc: %s already has channel %s", n->name, c->name);
        return false;
    }
    ...
    conoutf(CON_IRC, "\f9FanEd\f7::irc: %s added channel %s", n->name, d.name);
    return true;
}
...
ICOMMAND(ircpasschan, "sss", (const char *name, const char *chan, const char *s), {
    ...
    if(!c) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such %s channel: %s", n->name, chan); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s channel %s current password is: %s", n->name, c->name, c->passkey && *c->passkey ? "<set>" : "<not set>"); return; }
...
ICOMMAND(ircrelaychan, "sss", (const char *name, const char *chan, const char *s), {
    ...
    ircchan *c = ircfindchan(n, chan);
    if(!c) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such %s channel: %s", n->name, chan); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s channel %s current relay level is: %d", n->name, c->name, c->relay); return; }
    c->relay = parseint(s);
});
ICOMMAND(ircfriendlychan, "sss", (const char *name, const char *chan, const char *s), {
    ...
    ircchan *c = ircfindchan(n, chan);
    if(!c) { conoutf(CON_IRC, "\f9FanEd\f7::irc: no such %s channel: %s", n->name, chan); return; }
    if(!s || !*s) { conoutf(CON_IRC, "\f9FanEd\f7::irc: %s channel %s current friendly name is: %s", n->name, c->name, c->friendly); return; }
...
void ircprintf(ircnet *n, int relay, const char *target, const char *msg, ...)
{
    ...
    if(target && *target && strcasecmp(target, n->nick))
    {
        ...
        if(c)
        {
            formatstring(s)("\fs\f9[%s:%s]\fr", n->name, c->name);
            c->addline(str, MAXLINES);
            if(n->type == IRCT_RELAY && c->relay >= relay)
                server::sendservmsgf("\fs\f9[%s]\fr %s", c->friendly, str);
        }
        else
        {
            formatstring(s)("\fs\f9[%s:%s]\fr", n->name, target);
            n->addline(str, MAXLINES);
        }
    }
    else
    {
        formatstring(s)("\fs\f9[%s]\fr", n->name);
...
void ircchatprintf(ircnet *n, int relay, const char *target, const char *msg, ...)
{
	...
	conoutf("%s %s", s, str);
        if(strstr(str, irchighlight)) playsound(S_HIGHLIGHT);
    }
    else
    {
        formatstring(s)("\fs\f9[%s]\fr", n->name);
...
void ircprocess(ircnet *n, char *user[3], int g, int numargs, char *w[])
{
        ...
        if(numargs > g+2)
        {
                ...
                if(p-word > 0)
                {
                    ...
                    if(ismsg)
                    {
                        if(!strcasecmp(q, "ACTION"))
                            ircprintf(n, 1, g ? w[g+1] : NULL, "\f9FanEd\f7::irc: %s %s", user[0], r);
                        else
                        {
                            ircprintf(n, 4, g ? w[g+1] : NULL, "%s requests: %s %s", user[0], q, r);

                            if(!strcasecmp(q, "VERSION"))
                                ircsend(n, "NOTICE %s :\vVERSION Cube 2: Sauerbraten - Fanatic Edition\v", user[0]);
                            else if(!strcasecmp(q, "PING"))
                                ircsend(n, "NOTICE %s :\vPING %s\v", user[0], r);
                        }
                    }
                    else ircprintf(n, 4, g ? w[g+1] : NULL, "%s replied: %s %s", user[0], q, r);
                    DELETEA(q); DELETEA(r);
                }
            }
            else if(ismsg)
            {
                if(n->type == IRCT_RELAY && g && strcasecmp(w[g+1], n->nick) && !strncasecmp(w[g+2], n->nick, strlen(n->nick)))
                {
                    ...
                    if(p && *p) ircchatprintf(n, 0, w[g+1], "\f9<\f7%s\f9>\fR %s", user[0], p);
                }
                else ircchatprintf(n, 1, g ? w[g+1] : NULL, "\f9<\f7%s\f9>\f7 %s", user[0], w[g+2]);
            }
            else ircchatprintf(n, 2, g ? w[g+1] : NULL, "-%s- %s", user[0], w[g+2]);
        }
    }
    else if(!strcasecmp(w[g], "NICK"))
    {
        if(numargs > g+1)
        {
            ...
            ircprintf(n, 3, NULL, "%s (%s@%s) is now known as %s", user[0], user[1], user[2], w[g+1]);
        }
    }
    else if(!strcasecmp(w[g], "JOIN"))
    {
        if(numargs > g+1)
        {
            ...
            ircprintf(n, 3, w[g+1], "%s (%s@%s) has joined", user[0], user[1], user[2]);
        }
    }
    else if(!strcasecmp(w[g], "PART"))
    {
        if(numargs > g+1)
        {
            ...
            ircprintf(n, 3, w[g+1], "%s (%s@%s) has left", user[0], user[1], user[2]);
        }
    }
    else if(!strcasecmp(w[g], "QUIT"))
    {
        if(numargs > g+1) ircprintf(n, 3, NULL, "%s (%s@%s) has quit: %s", user[0], user[1], user[2], w[g+1]);
        else ircprintf(n, 3, NULL, "%s (%s@%s) has quit", user[0], user[1], user[2]);
    }
    else if(!strcasecmp(w[g], "KICK"))
    {
        if(numargs > g+2)
        {
            ...
            ircprintf(n, 3, w[g+1], "%s (%s@%s) has kicked %s from %s", user[0], user[1], user[2], w[g+2], w[g+1]);
        }
    }
    else if(!strcasecmp(w[g], "MODE"))
    {
        if(numargs > g+2)
        {
            ...
            ircprintf(n, 4, w[g+1], "%s (%s@%s) sets mode: %s %s", user[0], user[1], user[2], w[g+1], modestr);
        }
        else if(numargs > g+1)
            ircprintf(n, 4, w[g+1], "%s (%s@%s) sets mode: %s", user[0], user[1], user[2], w[g+1]);
    }
    ...
    else
    {
        ...
        if(numeric) switch(numeric)
        {
            case 1:
            {
                ...
                {
                    n->state = IRC_ONLINE;
                    ircprintf(n, 4, NULL, "now connected to %s as %s", user[0], n->nick);
            ...
            case 475:
            {
                ...
                if(c)
                {
                    ...
                    if(c->type == IRCCT_AUTO)
                        ircprintf(n, 4, w[g+2], "waiting 5 mins to rejoin %s", c->name);
                }
                break;
            }
            default: break;
        }
        if(s[0]) ircchatprintf(n, 4, targ, "%s %s", w[g], s);
...
void irccleanup()
{
    ...
    {
        ircnet *n = ircnets[i];
        ircsend(n, "QUIT :%s", ircquitmessage);
        ircdiscon(n);
    }
}
...
void irccmd(ircnet *n, ircchan *c, char *s)
{
    ...
    if(*p == '/')
    {
        ...
        if(p-word > 0)
        {
            ...
            if(!strcasecmp(q, "ME"))
            {
                if(c)
                {
                    ...
                    ircprintf(n, 1, c->name, "\f9*\f7 %s %s", n->nick, r);
                }
                else ircprintf(n, 4, NULL, "you are not on a channel");
            }
            ...
            else if(!strcasecmp(q, "PRIVMSG"))
            {
                ...
                {
                    ircsend(n, "%s %s", q, r);
                    ircprintf(n, 3, NULL, "\f9<\f7%s\f9>\f7 %s", n->nick, r);
                }
                else ircprintf(n, 4, NULL, "you are not on a channel");
            }
        ...
        ircprintf(n, 4, c ? c->name : NULL, "you are not on a channel");
    }
    else if(c)
    {
        ircsend(n, "PRIVMSG %s :%s", c->name, p);
        ircchatprintf(n, 1, c->name, "\f9<\f7%s\f9>\f7 %s", n->nick, p);
    }
    else
    {
        ircsend(n, "%s", p);
...
ICOMMAND(ircsay, "sss", (const char *net, const char *chan, const char *text), {
	if (!net[0] || !chan[0] || !text[0]) return;
	ircnet *in = ircfind(net);
	if (!in)
	{
		conoutf(CON_IRC, "\f9FanEd\f7::irc: you are not connected to an IRC network");
		return;
	}
	ircchan *ic = ircfindchan(in, chan);
	if (!in)
	{
		conoutf(CON_IRC, "\f9FanEd\f7::irc: you are not on a channel");
		return;
	}
