...
#include "irc.h"  // Fanatic Edition

extern int init_lua(); // Fanatic Edition
...
void writeinitcfg()
{
    stream *f = openutf8file("faned_init.cfg", "w");
    if(!f) return;
    f->printf("// Fanatic Edition: faned_init.cfg\n// Automatically created; do not modify.\n\n");
...
VARP(backgroundbluremphasis, 0, 4, 100);
VARP(backgroundblurrepeat, 0, 3, 100);
FVARP(backgroundfog, 0, 0.02, 1);
SVARP(backgroundimagefile, "faned/data/background.png");
SVARP(backgroundlogofile, "faned/data/logo.png");
...
void renderbackground(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    ...
    copystring(backgroundimg, backgroundimagefile);
    if(mapshot)
    {
        defformatstring(str)("<blur:%d/%d/0>packages/base/%s.jpg", backgroundbluremphasis, backgroundblurrepeat, mapname);
        copystring(backgroundimg, str);
    }

    if(!mainmenu) stopsounds();
    ...
    {
        glColor3f(1, 1, 1);
        settexture(backgroundimagefile, 0);
        glBegin(GL_QUADS);
        ...
        settexture(backgroundlogofile, 3);
        ...
        if(backgroundfog)
        {
            glColor4f(1, 1, 1, backgroundfog);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            settexture("faned/data/backgroundfog.png");
            glBegin(GL_QUADS);
            glTexCoord2f(0, fogmove); 
            glVertex2f(0, 0);
            glTexCoord2f(1.0f, fogmove);
            glVertex2f(w, 0);
            glTexCoord2f(1.0f, fogmove + 1.0f);
            glVertex2f(w, h);
            glTexCoord2f(0, fogmove + 1.0f);
            glVertex2f(0, h);
            glEnd();
            fogmove -= 0.0007f;
        }
        ...
        if(mapshot || mapname)
        {
            ...
            settexture("faned/data/mapshot_frame.png", 3);
            glBegin(GL_TRIANGLE_STRIP);
            glTexCoord2f(0, 0); glVertex2f(x,    y);
            glTexCoord2f(1, 0); glVertex2f(x+sz, y);
            glTexCoord2f(0, 1); glVertex2f(x,    y+sz);
...
void renderprogress(float bar, const char *text, GLuint tex, bool background)   // also used during loading
{
    if(seloadingscreen)
    {
        ...
        settexture("faned/data/circle1.png");
        glPushMatrix();
        ...
        settexture("faned/data/circle2.png");
        glPushMatrix();
        glTranslatef(w/2, h/2, 0);
        ...
        settexture("faned/data/circle3.png");
        glPushMatrix();
        glTranslatef(w/2, h/2, 0);
        ...
        glPushMatrix();
        glScalef(1/2.6f, 1/2.6f, 1);
        text_bounds(backgroundmap, wb, hb);
        x = ((w/2)*2.6f)-wb/2,
        y = (h/2)*2.99f;

        draw_text(backgroundmap, x, y-64);
        glPopMatrix();
    ...
    else
    {
        ...
        settexture("faned/data/loading_frame.png", 3); // Fanatic Edition
        glBegin(GL_TRIANGLE_STRIP);
        ...
        if(bar > 0)
        {
            settexture("faned/data/loading_bar.png", 3); // Fanatic Edition
            glBegin(GL_QUADS);
...
VAR(menufps, 0, 200, 1000); // Fanatic Edition
...
int main(int argc, char **argv)
{
    ...
    initserver(dedicated>0, dedicated>1);
    ASSERT(dedicated <= 1);
    game::initclient();
    ...
    SDL_WM_SetCaption("Cube 2: Sauerbraten - Fanatic Edition", NULL); // Fanatic Edition
    keyrepeat(false);
    SDL_ShowCursor(0);
    ...
    if(!execfile("data/stdlib.cfg", false)) fatal("cannot find data files (you are running from the wrong folder, try .bat file in the main folder)");
    ...
    // Start: Fanatic Edition
    logoutf("init: FanEd: config");
    if(!execfile(game::faned_config(), false))
    {
        execfile(game::faned_defaultconfig());
        writecfg(game::faned_restoreconfig());
    }
    
    logoutf("init: FanEd: scripts");
    execfile(game::faned_scripts(), false);

    logoutf("init: FanEd: sounds");
    execfile(game::faned_sounds(), false);

    logoutf("init: Faned: servers");
    if(game::faned_savedservers()) execfile(game::faned_savedservers(), false);

    logoutf("init: FanEd: history");
    loadhistory();

    logoutf("init: FanEd: whois");
    if(!fileexists(findfile(game::faned_whois(), "r"), "r"))
    {
        writedb(game::faned_whois());
    }
    // End: Fanatic Edition
    ...
    logoutf("init: lua");
    init_lua();
    execfile(game::faned_autoexec(), false);
    // End: Fanatic Edition
