...
namespace entities
{
    ...
    VARP(autoswitchweapon, 0, 0, 1);
    VARP(conoutf_healthboost, 0, 0, 1);
    VARP(conoutf_quaddamage, 0, 0, 1);
    ...
    void pickupeffects(int n, fpsent *d)
    {
        ...
        if(!d) return;
        //itemstat &is = itemstats[type-I_SHELLS]; // Fanatic Edition
        if(d!=player1 || isthirdperson())
        {
            //particle_text(d->abovehead(), is.name, PART_TEXT, 2000, 0xFFC864, 4.0f, -8); // Fanatic Edition
            //particle_icon(d->abovehead(), is.icon%4, is.icon/4, PART_HUD_ICON_GREY, 2000, 0xFFFFFF, 2.0f, -8); // Fanatic Edition
        }
        ...
        if(d==player1) switch(type)
        {
            ...
            case I_SHELLS:
                ...
                if(autoswitchweapon) switchweapon(type);

            break;
            // End: Fanatic Edition

            case I_BOOST:
                if(conoutf_healthboost) conoutf(CON_GAMEINFO, "\f2you have a permanent +10 health bonus! (%d)", d->maxhealth); // Fanatic Edition
                playsound(S_V_BOOST, NULL, NULL, 0, 0, 0, -1, 0, 3000);
                break;

            case I_QUAD:
                if(conoutf_quaddamage) conoutf(CON_GAMEINFO, "\f2you got the quad!"); // Fanatic Edition
    ...
    void teleporteffects(fpsent *d, int tp, int td, bool local)
    {
        if(ents.inrange(tp) && ents[tp]->type == TELEPORT)
        {
            ...
            if(e.attr4 >= 0) 
            {
                ...
                else playsound(snd, &ents[td]->o, NULL, flags);

                if(ents.inrange(td) && ents[td]->type == TELEDEST)
                {
                    adddynlight(ents[td]->o, 200, vec(0.5f, 0.5f, 1.0f), 500, 100, DL_FLASH, 0, vec(0, 0, 0), d);
                    if(d != player1 && player1->state != CS_SPECTATOR) particle_splash(PART_TELE, 1, 300, ents[td]->o, 0xFFFFFF, 20.0f, 1, 500);
                }
                // End: Fanatic Edition
            }
        }
