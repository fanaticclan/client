...
namespace ai
{
    ...
    void init(fpsent *d, int at, int ocn, int sk, int bn, int pm, const char *name, const char *team)
    {
        ...
        if(!d->name[0])
        {
            if(aidebug) conoutf("\f0connected:\f7 %s, assigned to %s at skill %d", colorname(d, name), o ? colorname(o) : "?", sk); // Fanatic Edition
            else conoutf("\f0connected:\f7 %s", colorname(d, name)); // Fanatic Edition
            resetthisguy = true;
        }
        else
        {
