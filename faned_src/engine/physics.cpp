...
ICOMMAND(floorz, "f", (float *f),
{
    ...
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::floorz: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else if(*f) floorz = *f;
...
ICOMMAND(gravity, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::gravity: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else if(*f) gravity = *f;
...
ICOMMAND(jumpvel, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::jumpvel: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else if(*f) jumpvel = *f;
...
ICOMMAND(slopez, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::slopez: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else if(*f) slopez = *f;
...
ICOMMAND(stairheight, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::stairheight: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
...
ICOMMAND(velocity, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::velocity: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else velocity = *f;
...
ICOMMAND(wallz, "f", (float *f),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::wallz: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else if(*f) wallz = *f;
...
ICOMMAND(jesus, "i", (int *i),
{
    if(!m_check(game::gamemode, M_EDIT))
    {
        conoutf(CON_ERROR, "\f9FanEd\f7::jesus: \f3#error: \f7operation only allowed in edit mode");
        playsound(S_ERROR);
        return;
    }
    else jesus = *i;
...
VARP(collisions, 0, 1, 1); // Fanatic Edition

bool plcollide(physent *d, const vec &dir)    // collide with player or monster
{
    if(d->type==ENT_CAMERA || d->state!=CS_ALIVE || (m_check(game::gamemode, M_EDIT) && !collisions)) return false; // Fanatic Edition
    loopdynentcache(x, y, d->o, d->radius)
    {
        const vector<physent *> &dynents = checkdynentcache(x, y);
        loopv(dynents)
...
static inline bool octacollide(physent *d, const vec &dir, float cutoff, const ivec &bo, const ivec &bs, const cube *c, const ivec &cor, int size) // collide with octants
{
    loopoctabox(cor, size, bo, bs)
    {
        ...
        else
        {
            bool solid = false;
            // Start: Fanatic Edition
            switch(c[i].material&MATF_VOLUME)
            {
                case MAT_WATER: solid = jesus ? true : false; break;
            }
            // End: Fanatic Edition
...
FVARP(maxroll, 0, 0, 20.f); // Fanatic Edition
FVAR(straferoll, 0, 0.033f, 90);
FVAR(faderoll, 0, 0.95f, 1);
FVARP(floatspeed, 0.0f, 100.0f, INT_MAX); // Fanatic Edition
...
void modifyvelocity(physent *pl, bool local, bool water, bool floating, int curtime)
{
    ...
    else if(pl->physstate >= PHYS_SLOPE || water || (m_check(game::gamemode, M_EDIT) && airjump))
    {
...
bool moveplayer(physent *pl, int moveres, bool local, int curtime)
{
    ...
    else                        // apply velocity with collision
    {
        ...
        if(timeinair > 1000 && !pl->timeinair && !water) // if we land after long time must have been a high jump, make thud sound // Fanatic Edition
        {
            game::physicstrigger(pl, local, -1, 0);
        }
    }
