...
VARP(fontfix, 0, 0, 1); // Fanatic Edition
...
ICOMMAND(setcampos, "fff", (float *x, float *y, float *z),
{
...
VARP(zoomfov, 10, 60, 90); // Fanatic Edition
VARP(fov, 10, 100, 150);
...
FVAR(thirdpersonup, -25, 3, 25); // Fanatic Edition
...
VARFP(minimapsize, 7, 10, 10, { if(minimaptex) drawminimap(); }); // Fanatic Edition
...
{
    if(zoom) return; // Fanatic Edition
...
{
    if(lastmillis >= damageblendmillis || zoom) return; // Fanatic Edition
...
void drawdeathcamerascreen(int w, int h)
{
    if(deathcamerascreen && deathcamerastate)
    {
        ...
        Texture *t = textureload("faned/hud/cam.png", 3);
...
VARP(deathpanicscreen, 0, 1, 1);

void drawdeathpanicscreen(int w, int h)
{
    if(deathpanicscreen && game::player1->health <= 25 && game::player1->state==CS_ALIVE && !m_check(game::gamemode, M_INSTA) && !zoom)
    {
        ...
        static Texture *deathpanictex = NULL;
        if(!deathpanictex) deathpanictex = textureload("faned/hud/veins_revelade_revolution.png", 3);
        glBindTexture(GL_TEXTURE_2D, deathpanictex->id);
...
VARP(zoomscope, 0, 0, 1);

void drawzoomscopescreen(int w, int h)
{
    if(zoomscope && game::player1->gunselect == GUN_RIFLE && zoom)
    {
        defaultshader->set();
        glEnable(GL_TEXTURE_2D);
        ...
        static Texture *zoomscopetex = NULL;
        if(!zoomscopetex) zoomscopetex = textureload("faned/hud/scope.png", 3);
        glBindTexture(GL_TEXTURE_2D, zoomscopetex->id);
        glColor3f(1, 1, 1);
...
FVARP(crosshairalpha, 0, 1.0f, 1.0f);
...
VARP(crosshairsize, 0, 22, 50);
VARP(cursorsize, 0, 22, 50);
...
void crosshairbump()
{
    if(!crosshairfx) return;
    if(csize == crosshairsize || !csize) csize = crosshairsize;
    if(cbump)
    {
        cbump = false;
        crosshairsize = csize;
...
void crosshairbumploop()
{
    if(cbump && csize == crosshairsize)
    {
        crosshairsize += 20;
        cmillis = lastmillis;
    }
    if(lastmillis >= (cmillis+10) && cbump)
    {
        crosshairsize -= 2;
        cmillis = lastmillis;
    }
    if(cbump && crosshairsize <= csize)
    {
        crosshairsize = csize;
        cbump = false;
        csize = 0;
...
void drawcrosshair(int w, int h)
{
    ...
    if(!windowhit && (!zoom && (hidehud || mainmenu))) return;
    float r = haircolor.x, g = haircolor.y, b = haircolor.z, cx = 0.5f, cy = 0.5f, chsize; // Fanatic Edition
    Texture *crosshair;
    if(windowhit)
    {
        // Start: Fanatic Edition
        static Texture *cam = NULL;
        if(!cam) cam = textureload("faned/hud/cam.png", 3, true);

        static Texture *scope = NULL;
        if(!scope) scope = textureload("faned/hud/scope.png", 3, true);
        // End: Fanatic Edition

        static Texture *cursor = NULL;
        if(!cursor) cursor = textureload("faned/data/guicursor.png", 3, true);
    ...
    { 
        int index = game::selectcrosshair(r, g, b, w, h); // Fanatic Edition
        if(index < 0) return;
    ...
    glColor4f(r, g, b, crosshairalpha);
...
string buildversion = "20141026223022-DEV";
...
void gl_drawhud(int w, int h)
{
    ...
    if(!mainmenu)
    {
        crosshairbumploop();         // Fanatic Edition
        drawdamagescreen(w, h);
        drawdamagecompass(w, h);
        drawdeathcamerascreen(w, h); // Fanatic Edition
        drawdeathpanicscreen(w, h);  // Fanatic Edition
        drawzoomscopescreen(w, h);   // Fanatic Edition
    }
    ...
    else conw = int(w/conscale), conh = int(h/conscale), abovehud = conh - FONTH, limitgui = abovehud-200;

    if(mainmenu && showbuildversion)
    {
        glPushMatrix();
        glScalef(1/3.5f, 1/3.5f, 1);
        int tw, th;
        text_bounds(buildversion, tw, th);
        draw_textf("\f7%s", 30, h*3.5f-th-20, buildversion);
        glPopMatrix();
    }
    // End: Fanatic Edition
    ...
    if(!hidehud && !mainmenu)
    {
        if(!hidestats)
        {
            ...
            if(showfps && !sehud) // Fanatic Edition
            {
                ...
                draw_textf("fps %d", conw-5*FONTH, conh-FONTH*3/2-roffset, curfps[0]);
                roffset += FONTH;
            }

            if(wallclock && !sehud) // Fanatic Edition
            {
        ...
        if(hidestats || (!editmode && !showeditstats))
        {
            ...
            if(!sehud) abovehud = min(abovehud, int(conh*game::abovegameplayhud(w, h))); // Fanatic Edition
        }
    ...
    // drawcrosshair(w, h); // Fanatic Edition
