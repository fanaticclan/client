...
{      
    ...
    VARFP(playermodel, 0, 1, 5, changedplayermodel()); // Fanatic Edition
    ...
    static const playermodelinfo playermodels[6] = // Fanatic Edition
    {
        ...
        { "ogro2", "ogro2/blue", "ogro2/red", "ogro2/hudguns", NULL, "ogro2/quad", { "ogro2/armor/blue", "ogro2/armor/green", "ogro2/armor/yellow" }, "ogro", "ogro_blue", "ogro_red", true },
        ...
        { "sgt.ross", "sgt.ross/blue", "sgt.ross/red", "sgt.ross/hudguns", NULL, "sgt.ross/quad", { "sgt.ross/armor/blue", "sgt.ross/armor/green", "sgt.ross/armor/yellow" }, "sgt.ross", "sgt.ross_blue", "sgt.ross_red", true } // Fanatic Edition
    };
    ...
    VAR(joinred, 0, 0, 1);
    ICOMMAND(teamred, "", (), { joinred = 1; });
    ICOMMAND(teamblue, "", (), { joinred = 0; });
    // End: Fanatic Edition
    ...
    void rendergame(bool mainpass)
    {
        ...
        loopv(players)
        {
            ...
            int team = 0;
            if(teamskins || m_teammode) // Fanatic Edition
            {
                if(!joinred) team = isteam(player1->team, d->team) ? 1 : 2;
                else team = isteam(player1->team, d->team) ? 2 : 1;
            }
            ...
            if(d->state!=CS_DEAD) particle_text(d->abovehead(), d->info, PART_TEXT, 1, team ? (team==1 ? 0x6496FF : 0xFF4B19) : 0x1EC850, clamp(camera1->o.dist(d->abovehead()), 200.0f, 400.0f)/100.f); // Fanatic Edition
        }
        loopv(ragdolls)
        {
            fpsent *d = ragdolls[i];
        ...
        {
            if(isthirdperson() && !followingplayer() && (player1->state!=CS_DEAD || !hidedead)) renderplayer(player1, getplayermodelinfo(player1), teamskins || m_teammode ? 2 : 0, 1, mainpass);
        }
        // End: Fanatic Edition
        rendermonsters();
        rendermovables();
    ...
    SVARP(hudgunsdir, "");
    VARP(hudgunalpha, 0, 255, 255); // Fanatic Edition
    ...
    void drawhudmodel(fpsent *d, int anim, float speed = 0, int base = 0)
    {
        ...
        {
            if(!joinred) concatstring(gunname, d==player1 || isteam(d->team, player1->team) ? "/blue" : "/red");
            else concatstring(gunname, d==player1 || isteam(d->team, player1->team) ? "/red" : "/blue");
        }
        // End: Fanatic Edition
        else if(testteam > 1)
        ...
        rendermodel(NULL, gunname, anim, sway, testhudgun ? 0 : d->yaw+90, testhudgun ? 0 : d->pitch, MDL_LIGHT|MDL_HUD, interp, a, base, (int)ceil(speed), (float)hudgunalpha/255); // Fanatic Edition
    ...
    {
        fpsent *d = hudplayer();
        if(hudgun && !(getvar("zoom") && getvar("zoomscope") && (d->gunselect == GUN_RIFLE)))
        {
    ...
    void preloadweapons()
    {
        ...
        loopi(NUMGUNS)
        {
            ...
            if(!file) continue;
            string fname, fnameblue, fnamered;
            if((m_teammode || teamskins) && teamhudguns)
            {
                // Start: Fanatic Edition
                formatstring(fnameblue)("%s/%s/blue", hudgunsdir[0] ? hudgunsdir : mdl.hudguns, file);
                preloadmodel(fnameblue);
                formatstring(fnamered)("%s/%s/red", hudgunsdir[0] ? hudgunsdir : mdl.hudguns, file);
                preloadmodel(fnamered);
                // End: Fanatic Edition
            }
            else
            {
