...
void rendermodel(entitylight *light, const char *mdl, int anim, const vec &o, float yaw, float pitch, int flags, dynent *d, modelattach *a, int basetime, int basetime2, float trans)
{
    ...
    else if(showboundingbox && !shadowmapping && !reflecting && !refracting)    // Fanatic Edition
    {
...
VARP(thirdpersonalpha, 0, 100, 100); // Fanatic Edition

void renderclient(dynent *d, const char *mdlname, modelattach *attachments, int hold, int attack, int attackdelay, int lastaction, int lastpain, float fade, bool ragdoll, bool thirdperson)
{
    ...
    if(d == game::player1) fade = thirdpersonalpha/100.0f;
