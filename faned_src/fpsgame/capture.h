...
#endif
{
        ...
        if(!joinred)
        {
            ...
            if(d->state==CS_ALIVE)
            {
                loopv(bases)
                {
                    ...
                    if(!b.valid() || !insidebase(b, d->feetpos()) || (strcmp(b.owner, d->team) && strcmp(b.enemy, d->team))) continue;
                    if(d->lastbase < 0 && (lookupmaterial(d->feetpos())&MATF_CLIP) == MAT_GAMECLIP) break;
                    particle_flare(pos, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - AMMOHEIGHT - 4.4f), 0, PART_LIGHTNING, strcmp(d->team, player1->team) ? 0x2222FF : 0xFF2222, 1.0f);
                    if(oldbase < 0)
                    {
                        particle_fireball(pos, 4.8f, PART_EXPLOSION, 250, strcmp(d->team, player1->team) ? 0x2020FF : 0x802020, 4.8f);
                        particle_splash(PART_SPARK, 50, 250, pos, strcmp(d->team, player1->team) ? 0x2020FF : 0x802020, 0.24f);
                    }
                    d->lastbase = i;
                }
            }
    ...
    void rendergame()
    {
        ...
        loopv(bases)
        {
            ...
            // Start: Fanatic Edition
            const char *basename;
            if(!joinred) basename = b.owner[0] ? (strcmp(b.owner, player1->team) ? "base/red" : "base/blue") : "base/neutral";
            else basename = b.owner[0] ? (strcmp(b.owner, player1->team) ? "base/blue" : "base/red") : "base/neutral";
            // End: Fanatic Edition
            ...
            // Start: Fanatic Edition
            if(!joinred) regular_particle_flame(PART_FLAME, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - 4.5f), fradius, fheight, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 3, 2.0f);
            else regular_particle_flame(PART_FLAME, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - 4.5f), fradius, fheight, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x2020FF : 0x802020) : 0x208020, 3, 2.0f);
            // End: Fanatic Edition
...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
            ...
            if(!joinred)
            {
                ...
                if(b.owner[0])
                {
                    ...
                    tcolor = isowner ? 0xFF4B19 : 0x6496FF;
                }
                else if(b.enemy[0])
                {
                    if(!b.name[0]) formatstring(b.info)("base %d: %s", i+1, b.enemy);
                    ...
                    if(strcmp(b.enemy, player1->team)) { tcolor = 0x6496FF; mtype = PART_METER; mcolor = 0x3219FF; }
                    else { tcolor = 0xFF4B19; mtype = PART_METER; mcolor = 0xFF1932; }
                }
                else if(!b.name[0]) formatstring(b.info)("base %d", i+1);
                else if(basenumbers) formatstring(b.info)("%s (%d)", b.name, i+1);
                else copystring(b.info, b.name);
    ...
    void setscore(int base, const char *team, int total)
    {
        ...
        else if(bases.inrange(base))
        {
            ...
            if(!strcmp(b.owner, team))
            {
                ...
                // Start: Fanatic Edition
                if(!joinred) particle_textcopy(above, msg, PART_TEXT, 2000, isteam(team, player1->team) ? 0x6496FF : 0xFF4B19, 4.0f, -8);
                else particle_textcopy(above, msg, PART_TEXT, 2000, isteam(team, player1->team) ? 0xFF4B19 : 0x6496FF, 4.0f, -8);
                // End: Fanatic Edition
            }
        }
    }
