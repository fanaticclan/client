...
SVARP(moviedir, "movie"); // Fanatic Edition
...
struct aviwriter
{
    ...
    bool open()
    {
        ...
        listchunk("LIST", "INFO");
        const char *software = "Cube 2: Sauerbraten - Fanatic Edition"; // Fanatic Edition
...
namespace recorder
{
    ...
    void start(const char *filename, int videofps, int videow, int videoh, bool sound) 
    {
        ...
        if(!file->open()) 
        { 
            conoutf(CON_ERROR, "unable to create file %s", file->filename); // Fanatic Edition
            DELETEP(file);
            return;
        }
