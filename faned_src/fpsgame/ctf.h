...
#endif
{
    ...
    void rendergame()
    {
        loopv(flags)
        {
            ...
            {
                // Start: Fanatic Edition
                if(!joinred) particle_flare(pos, f.spawnloc, 0, PART_LIGHTNING, strcmp(f.owner->team, player1->team) ? 0xFF2222 : 0x2222FF, 1.0f);
                else particle_flare(pos, f.spawnloc, 0, PART_LIGHTNING, strcmp(f.owner->team, player1->team) ? 0x2222FF : 0xFF2222, 1.0f);
                if(!flags.inrange(f.owner->lastbase))
                {
                    ...
                    {
                        particle_fireball(pos, 4.8f, PART_EXPLOSION, 250, strcmp(f.owner->team, player1->team) ? 0x2020FF : 0x802020, 4.8f);
                        particle_splash(PART_SPARK, 50, 250, pos, strcmp(f.owner->team, player1->team) ? 0x2020FF : 0x802020, 0.24f);
                    }
                }
                // End: Fanatic Edition
                f.owner->lastbase = i;
    ...
    void dropflag(fpsent *d, int i, int version, const vec &droploc)
    {
        ...
        // Start: Fanatic Edition
        if(strstr(ctfsoundset, "sauerbraten")) playsound(S_FLAGDROP);
    ...
    void flagexplosion(int i, int team, const vec &loc)
    {
        ...
        {
            if(!team) { fcolor = 0xFF8080; color = vec(1, 0.75f, 0.5f); }
            else if(team==ctfteamflag(player1->team)) { fcolor = 0x802020; color = vec(1, 0.25f, 0.25f); }
    ...
    void flageffect(int i, int team, const vec &from, const vec &to)
    {
        ...
        {
            // Start: Fanatic Edition
            if(!joinred) particle_flare(fromexp, toexp, 600, PART_LIGHTNING, !team ? 0xFFC0A0 : (team==ctfteamflag(player1->team) ? 0x2222FF : 0xFF2222), 1.0f);
            else particle_flare(fromexp, toexp, 600, PART_LIGHTNING, !team ? 0xFFC0A0 : (team==ctfteamflag(player1->team) ? 0xFF2222 : 0x2222FF), 1.0f);
            // End: Fanatic Edition
        }
    }
    ...
    void returnflag(fpsent *d, int i, int version)
    {
        ...
        // Start: Fanatic Edition
        if(strstr(ctfsoundset, "sauerbraten")) playsound(S_FLAGRETURN);
    ...
    void resetflag(int i, int version, int spawnindex, int team, int score)
    {
        ...
        if(shouldeffect)
        {
            ...
            // Start: Fanatic Edition
            if(strstr(ctfsoundset, "sauerbraten")) playsound(S_FLAGRESET);
    ...
    void scoreflag(fpsent *d, int relay, int relayversion, int goal, int goalversion, int goalspawn, int team, int score, int dflags)
    {
        ...
        // Start: Fanatic Edition
        if(strstr(ctfsoundset, "sauerbraten")) playsound(team==ctfteamflag(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);
    ...
    void takeflag(fpsent *d, int i, int version)
    {
        ...
        // Start: Fanatic Edition
        if(strstr(ctfsoundset, "sauerbraten")) playsound(S_FLAGPICKUP);
