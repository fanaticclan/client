...
ICOMMAND(abort, "", (), abort());

ICOMMAND(system, "s", (char *cmd), {
    char * command = cmd;
    int ret = system(command);
    if(ret == -1) conoutf("\f9FanEd\f7::system: \f3#error.\f4-1\f7.");
    else if( ret > 0 ) conoutf("\f9FanEd\f7::system: Exit: \f4%d\f7.", ret);
    else conoutf("\f9FanEd\f7::system: \f4%s\f7 executed.", cmd);
});

ICOMMAND(date, "s", (char *format),
{
    string buf;
    time_t t = time(NULL);
    size_t len = strftime(buf, sizeof(buf), format, localtime(&t));
...
// Start: Fanatic Edition
void writedb(const char *name)
{
    stream *f = openutf8file(path(name && name[0] ? name : game::faned_whois(), true), "w");
    if(!f) return;
    f->printf("// Fanatic Edition: faned_whois.db\n// Automatically created; do not modify.\n\n");
    delete f;
}
COMMAND(writedb, "s");
...
{
    stream *f = openutf8file(path(name && name[0] ? name : game::faned_config(), true), "w");
    if(!f) return;
    f->printf("// Fanatic Edition: faned_config.cfg\n// Automatically created; do not modify.\n\n");
...
ICOMMAND(<=s, "ss", (char *a, char *b), intret(strcmp(a,b)<=0));
ICOMMAND(>=s, "ss", (char *a, char *b), intret(strcmp(a,b)>=0));
ICOMMAND(echo, "C", (char *s), conoutf("\f7%s", s));
