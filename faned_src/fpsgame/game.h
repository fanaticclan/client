...
enum
{
    ...
    S_SET_RE_FLAGPICKUPENEMY,
    S_SET_RE_FLAGPICKUPTEAM,
    S_SET_RE_FLAGDROP,
    S_SET_RE_FLAGRETURN,
    S_SET_RE_FLAGSCORE,
    S_SET_RE_FLAGRESET,
...
struct fpsent : dynent, fpsstate
{
    ...
    ~fpsent()
    {
        ...
        if(hurtchan >= 0) stopsound(S_HEARTBEAT, hurtchan);    // Fanatic Edition
        if(ai) delete ai;
    }
    ...
    void stopheartbeat()    // Fanatic Edition
    {
...
namespace game
{
    ...
    extern int getteamscore(const char *team); // Fanatic Edition
    extern int joinred; // Fanatic Edition
}
