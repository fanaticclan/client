...
static bool texturedata(ImageData &d, const char *tname, Slot::Tex *tex = NULL, bool msg = true, int *compress = NULL)
{
    ...
    if(!tname)
    {
        ...
        if(tex->name[0]=='<') 
        {
            ...
            if(!file) { if(msg) conoutf(CON_WARN, "could not load texture packages/%s", tex->name); return false; } // Fanatic Edition
            file++;
        }
        else file = tex->name;
    ...
    else if(tname[0]=='<') 
    {
        cmds = tname;
        file = strrchr(tname, '>');
        if(!file) { if(msg) conoutf(CON_WARN, "could not load texture %s", tname); return false; } // Fanatic Edition
        file++;
    }
    ...
    if(flen >= 4 && (!strcasecmp(file + flen - 4, ".dds") || (dds && !raw)))
    {
        ...
        {
            if(msg) conoutf(CON_WARN, "could not load texture %s", dfile); // Fanatic Edition
            return false;
        }
    ...
    if(!d.data)
    {
        ...
        if(bpp%8 || !texformat(bpp/8)) { SDL_FreeSurface(s); conoutf(CON_WARN, "texture must be 8, 16, 24, or 32 bpp: %s", file); return false; } // Fanatic Edition
        if(max(s->w, s->h) > (1<<12)) { SDL_FreeSurface(s); conoutf(CON_WARN, "texture size exceeded %dx%d pixels: %s", 1<<12, 1<<12, file); return false; } // Fanatic Edition
        d.wrap(s);
    }
...
void screenshot(char *filename)
{
    ...
    playsound(S_SCREENSHOT); // Fanatic Edition
