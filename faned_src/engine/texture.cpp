...
static bool texturedata(ImageData &d, const char *tname, Slot::Tex *tex = NULL, bool msg = true, int *compress = NULL)
{
    ...
    if(flen >= 4 && (!strcasecmp(file + flen - 4, ".dds") || dds))
    {
        ...
        if(!dds || dbgdds) { if(msg) conoutf(CON_WARN, "could not load texture %s", dfile); return false; } // Fanatic Edition
    }
        
    SDL_Surface *s = loadsurface(file);
    if(!s) { if(msg) conoutf(CON_WARN, "could not load texture %s", file); return false; } // Fanatic Edition
