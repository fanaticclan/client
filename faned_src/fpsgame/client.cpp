...
namespace game
{
    ...
    int whois(void* data)
    {
        ...
        if(!client)
        {
            conoutf(CON_ERROR, "\f9FanEd\f7::whois: \f3#error: \f7client not found");
            playsound(S_ERROR);
            return 1;
        }
        ENetSocket whoissock;
        ...
        if(connectedpeer())
        {
            ...
            )
            {
                ...
                )
            	{
                    ...
                    if(!fr)
                    {
                        conoutf(CON_ERROR, "\f9FanEd\f7::whois: \f3#error: \f7failed to open \f4%s\f7 for reading", faned_whois());
                        playsound(S_ERROR);
                        return 1;
                    }
                    ...
                    char linebuf[30];
                    bool found = false;
                    extern bool duplicatename(fpsent *d, const char *name = NULL, const char *alt = NULL);

                    while(fr->getline(linebuf, sizeof(linebuf)))
                    {
                        if(strstr(linebuf, curip))
                        {
                            found = true;
                            char *info = strtok(linebuf, "\n");
                            conoutf(CON_INFO, "\f9FanEd\f7::whois: %s \f%d(%d)\f7, IP: \f4%s\f7; found entry \f9#%d\f7: \f4%s",
                                duplicatename(client, client->name) ? client->name : colorname(client),
                                duplicatename(client, client->name) ? 5 : 4,
                                client->clientnum,
                                curip,
                                i,
                                info
                    ...
                    if(found == false)
                    {
                        conoutf(CON_INFO, "\f9FanEd\f7::whois: %s \f%d(%d)\f7, IP: \f4%s",
                            duplicatename(client, client->name) ? client->name : colorname(client),
                            duplicatename(client, client->name) ? 5 : 4,
                            client->clientnum,
                            curip
    ...
    int whowas(void* data)
    {
        char *client = (char*)data;
        stream *fr = openutf8file(path(faned_whois(), true), "r");
        if(!fr)
        {
            conoutf(CON_ERROR, "\f9FanEd\f7::whowas: \f3#error: \f7failed to open \f4%s\f7 for reading", faned_whois());
            playsound(S_ERROR);
            return 1;
        }
        ...
        while(fr->getline(linebuf, sizeof(linebuf)))
        {
            if(strstr(linebuf, client))
            {
                found = true;
                char *info = strtok(linebuf, "\n");
                conoutf(CON_INFO, "\f9FanEd\f7::whowas: \f4%s\f7; found entry \f9#%d\f7: \f4%s", client, i, info);
            }
            i++;
        }
        if(found == false) conoutf(CON_INFO, "\f9FanEd\f7::whowas: \f4%s\f7; no entries.", client);
        else found = false;
        fr->close();
        return 0;
    }
